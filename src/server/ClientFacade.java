...
public class ClientFacade {
    ...
    public void handleLogout() {
        ...
        Server.removePublicKey(clientHandler.getUsername());
    ...
    public ResponseMessage handleGameStart() {
        ResponseMessage startResponse = getGuessingGame().startGame();
    ...
    public Message handlePublicKeyRequest(String targetUsername) {
        if (clientHandler.getUsername() == null) {
            // User is not logged in
            return new ResponseMessage("ENCRYPTED_MESSAGE_SEND_RESP", "ERROR", "6000");
        }
    ...
    public ResponseMessage handleSessionKeyExchange(SessionKeyExchangeRequestMessage requestMessage, String senderUsername) {
        if (!Server.isLoggedIn(senderUsername)) {
            return new ResponseMessage("ENCRYPTED_MESSAGE_SEND_RESP", "ERROR", "6000");
        }
        ...
        if (receiverHandler != null) {
            ...
            return new ResponseMessage("SESSION_KEY_EXCHANGE_RESP", "OK");
        } else {
            // Recipient not found
            return new ResponseMessage("SESSION_KEY_EXCHANGE_RESP", "ERROR", "11001");
        }
    }
